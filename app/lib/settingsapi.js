...
settingsApi.get_cache = function(username, domain, permission) {
   ...
   master_list.forEach(function(filename) {
      ...
      if (utils.fileExists(file_path) && path.extname(filename) == ".json" && filename != "vectorLayers.json" && filename != "SOSLayers.json" && filename.substring(filename.length - 17, filename.length) != "_walkthrough.json") {
