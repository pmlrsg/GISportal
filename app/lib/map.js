...
/**
 * Produce a video map from a single WMS data layer
 ...
 */
map.map = function(plotRequest, plotDir, downloadDir, logDir, next) {
   console.log('MADE IT TO MAPPING - SERVER');
   console.log('REQUEST:',plotRequest);
   ...
   readStatus(function(status) {
      if (!status || status.state == PlotStatus.failed) {
         ...
         if (dataOptions.bbox.substr(0, 7) == 'POLYGON') {
            return handleError('map doesn\'t support irregular polygons');
         }
   ...
   function downloadTiles(next) {
      ...
      function setupTimeStamper() {
         timeStamper = child_process.fork(path.join(__dirname, '../scripts/map-timestamper.js'));
         ...
         timeStamper.on('message', function(options) {
            ...
            fs.rename(options.tempPath, options.filePath, function(err) {
               ...
               // Link the image in the hashdir (NOT symlink to avoid issues with simultaneous similar maps)
      ...
      function downloadComplete(err, options) {
         ...
         if (options.id == 'map') {
            ...
            if (options.existing) {
               ...
               // (NOT symlink to avoid issues with simultaneous similar maps)
   ...
   function logComplete(next) {
      if (logDir) {
         ...
         var line = [datetime, hash, 'map', status.state, slices.length];
         line = line.join(',') + '\n';
