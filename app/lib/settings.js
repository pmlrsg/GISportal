...
settings.read_project_video=function(req,res){
   var domain = utils.getDomainName(req)
   if (GLOBAL.config[domain]['projectSpecific'].videoFile){
      try {
         var filePathForVideo=GLOBAL.config[domain]['projectSpecific'].videoFile;
         var videoPath = path.join(MASTER_CONFIG_PATH, domain,PROJECT_CUSTOMISATION_PATH, filePathForVideo);
         try {
            ...
            console.log('Error reading the video file',e)
            res.status(404).send('There was an issue reading the video file');
            return
         }
         
      } catch (e) {
         console.log('Error returning video location', e);
         res.status(404).send('Not found');
      }
   }
   else{
