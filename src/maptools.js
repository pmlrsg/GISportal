...
gisportal.maptools.init = function() {
   ...
   $.get('templates/maptools.mst', function(template) {
      ...
      $('#select-basemap').change(function() {
      	...
      	portalevent.trigger('displayoptions.basemap', ['select-basemap', $(this).val(), 'Base map changed to '+ $('#select-basemap option:selected').text() ])
      });
      ...
      // set an action for the country borders select changing
	   $('#select-country-borders').change(function() {
			...
			portalevent.trigger('displayoptions.basemap', ['select-country-borders', $(this).val(), 'Country borders set to \''+ $('#select-country-borders option:selected').text() +'\'' ])
		});
