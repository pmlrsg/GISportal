...
gisportal.TimeLine.prototype.addTimeBar = function(name, id, label, startDate, endDate, dateTimes) {
    ...
    var count_temporal_layers = 0;
    for (var i = 0; i < gisportal.selectedLayers.length; i++) {
        if (gisportal.layers[gisportal.selectedLayers[i]].temporal) {
            count_temporal_layers++;
        }
    }

    if (count_temporal_layers === 1 && (!gisportal.cache.state || !gisportal.cache.state.timeline)) {
        this.reHeight();
