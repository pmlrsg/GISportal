   ...
   $.ajax({
      ...
      success: function(old_data){
         ...
         for (var i_old = 0; i_old < old_data.server.Layers.length; i_old++) {
		  ...
		  new_data.server[new_server][i_new].rgb_check = old_data.server.Layers[i_old].rgb_check;
		  new_data.server[new_server][i_new].rgb_type = old_data.server.Layers[i_old].rgb_type;
