...
gisportal.indicatorsPanel.initDOM = function() {
   $('.js-indicators').on('click', '.js-animate', function() {
      // window.alert("Animation!");

      var id = $(this).data('id');
      gisportal.indicatorsPanel.animate(id);
   });
...
gisportal.indicatorsPanel.animate = function(id) {
   gisportal.panelSlideout.openSlideout('animate');
   var indicator = gisportal.layers[id];
   var rendered = gisportal.templates.animate({
      indicator: indicator
   });

   var content = $('.js-animate-slideout  .js-slideout-content')
      .html(rendered);
   ...
   content.find('.js-render').click(function() {
      var times = gisportal.layers[id].DTCache;
      var start = new Date($('.js-animate-slideout .js-min').val());
      var end = new Date($('.js-animate-slideout .js-max').val());
      ...
      var url = gisportal.layers[id].openlayers.anID.getSource().getUrls()[0];
      var params = gisportal.layers[id].openlayers.anID.getSource().getParams();

      $.ajax({
         method: 'post',
         url: gisportal.middlewarePath + '/animate',
         contentType: 'application/json',
         data: JSON.stringify({
            id: id,
            url: url,
            params: params,
            slices: slicesInRange
         }),
         dataType: 'json',
      ...
      //       method: "POST",
      //       data: {
      //          'data': JSON.stringify(download_data.data)
      //       },
      //       success: function(data) {
