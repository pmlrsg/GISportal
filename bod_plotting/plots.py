...
"""
Edited by BOD 07/2022 to allow direct python call from flask app. 
...
def extract(plot, outfile="image.html"):
   ...
   # try: # THIS WAS COMMENTED OUT AFTERWARDS
   #    os.mkdir(csv_dir)
   ...
   # csv_file = csv_dir + "/" + df['coverage'] + ".csv" # THIS WAS COMMENTED OUT AFTERWARDS
   #np.savetxt(csv_file, np.transpose(data), comments='', header=','.join(df['vars']), fmt="%s",delimiter=",") # THIS WAS FOUND COMMENTED OUT

   #with zipfile.ZipFile(csv_dir+".zip", mode='w') as zf: # THIS WAS FOUND COMMENTED OUT
      #zf.write(csv_file) # THIS WAS FOUND COMMENTED OUT

   #shutil.rmtree(csv_dir) # THIS WAS FOUND COMMENTED OUT
...
def hovmoller(plot, outfile="image.html"):
   print('OUTFILE IN HOVMOLLER: ',outfile)
   ...
   if plotting.debug.verbosity > 0:
      print('IN HERE DUE TO DEBUG VERBOSITY: ',plotting.debug.verbosity)
      output_file(outfile, 'Hovmoller')
      save(p)
   else:
      print('IN ELSE DUE TO DEBUG VERBOSITY: ',plotting.debug.verbosity)
      # with open(outfile, 'w') as ofile:
...
def timeseries(plot, outfile="time.html"):
   ...
   script, div = components(ts_plot)
   # print('SCRIPT AND DIV BELOW: ',script,div)
   ...
   if plotting.debug.verbosity > 0:
      ...
      # print('TIME SERIES NO DEBUG HERE')
      # with open(outfile, 'w') as ofile:
      #    # print(template.render(script=script, div=div), file=ofile)
      #    output=template.render(script=script, div=div)
      #    print(output)
      #    ofile.write(output)
      output_file(outfile, 'Time Series') # @TODO When template rendered explictly above the html shows no plot. When save() function is used the html displays plot. Do we need to be explicit here 
      save(ts_plot)
   return(1)
#END timeseries   
...
def get_plot_data(json_request, plot=dict(), download_dir="/tmp/"):
   # print("Inside GET_PLOT_DATA, plot next", plot) # Replace all print satements with debug. 
   irregular = False
   ...
   if plot_type in ("hovmollerLat", "hovmollerLon"):
      ...
      for s in series:
         ...
         data_request = "BasicExtractor('{}',{},extract_area={},extract_variable={})".format(ds['threddsUrl'], time_bounds, bbox, coverage)
         print('TIMESERIES DATA REQUEST: ',data_request)
         debug(3, u"Requesting data: {}".format(data_request))
         try:
            if irregular:
               bounds = wkt.loads(bbox).bounds
               ...
               extractor = BasicExtractor(ds['threddsUrl'], time_bounds, extract_area=bbox, extract_variable=coverage, extract_depth=depth, outdir=download_dir)
               print('Extractor basic here: ',extractor)
               print('Extractor basic inards: ',ds['threddsUrl'], time_bounds, bbox,coverage, depth,download_dir)

            extract = extractor.getData()
            print('extract IN TIMESERIES ',extract)
            ts_stats = BasicStats(extract, coverage, isLog)
            print('TS_STATS IN TIMESERIES ',ts_stats)
            response = json.loads(ts_stats.process())
            print('RESPONSE IN TIMESERIES ',response)
         except ValueError:
...
def prepare_plot_flask(request, outdir): 
   '''
   Prepare_plot takes a plot request and hashes it to produce a key for future use.
   If all looks OK it returns the hash to the flask app.
   Then the execute_plot function is called to get the plot processing completed  
   simultaneously so that future calls to read the status-json return the progress
   '''
   ...
   download_dir=r'./tmp/'
   base_url='TEST_URL' # @TODO Give this an appropriate URL as per Dev Portal 

   fp1=multiprocessing.Process(target=execute_plot,args=[outdir, plot, request, base_url, download_dir]) #fp stands for flask plotting
   fp1.start()
   #
   #
...
def execute_plot(dirname, plot, request, base_url, download_dir):
   ...
   print(u"Received request: {}".format(request))
   ...
   if status == None or status['state'] == Plot_status.failed:
      print('Inside this if statement - calling get_plot_data')
      new_plot = True
      ...
      request_path = dirname + "/" + my_hash + "-request.json"
      print('FILE PATH FOR REQUEST PRINTED HERE: ',request_path)
      print(u"File: {}".format(request_path))
      ...
      print('Inside this else function')
      print(my_hash)
      return True
   ...
   file_path = dirname + my_hash + "-plot.html"
   # print('EXECUTE PLOT, PLOT PRINTED HERE: ',plot)
   ...
   update_status(dirname, my_hash, Plot_status.complete, "Complete", base_url=base_url)
   return True
#END execute_plot
...
# Now that the Flask App calls this module directly, we can't rely on the  section above

# @TODO Replacing print statements with debug
# @TODO Implement Logging statements


   def python_plot_from_flask(hash,dirname,url,download_dir):

      request_file_path=join(dirname,(hash+'-request.json'))
      
      with open(request_file_path) as f:
         request=json.load(f)
      
      # print('REQUEST INSIDE HERE: ',request)

      plot = prepare_plot(request, dirname)

      print('Returned PLOT here',plot)

      my_hash = plot['req_hash']


      execute_plot(dirname, plot, request, url,download_dir)
      # try:
      #    if execute_plot(dirname, plot, request, url, download_dir):
      #       print("Plot complete")
      #    else:
      #       print("Error executing. Failed to complete plot")
      #       # logger.log_complete(False)
      #       # sys.exit(2)
      # except:
      #    trace_message = traceback.format_exc()
      #    print(u"Uncaught Exception. Failed to complete plot - {}".format(trace_message))
      #    # update_status(opts.dirname, my_hash, Plot_status.failed, "Extract failed", traceback=trace_message) @TODO Import update status from animation.py and then update here
      #    # logger.log_complete(False)
      #    raise
   # hash='fe221792f17e037037c9a7847b7fa863b46a3565'
   # hash='8b702676ea06e0bec796e24738a0f82f2bdcc57f'
   # hash='7f181d19d1ec83f850c73f7ca2049c2efe5821b3'
   # hash='039305083e4920576f42bc42401344c63b47d1b9'
   # hash='20b94a96fb083f2d1618cbb2a9138544a6b6c210'
   # hash='447b92235fe441d7b57b838ac6389228f09959bf'

   dirname='./plots/'
   download_dir='./tmp/'
   url=''

   # python_plot_from_flask(hash,dirname,url,download_dir)
   ...
   # name_list=['20b94a96fb083f2d1618cbb2a9138544a6b6c210']

   plot_parrellel=[]

   for hash in name_list:
      parrallel_process=multiprocessing.Process(target=python_plot_from_flask,args=[hash,dirname,url,download_dir])
      parrallel_process.start()
      plot_parrellel.append(parrallel_process)
   
   for processes in plot_parrellel:
      processes.join()

   # url='LOCALHOSTSOMETHING' 
