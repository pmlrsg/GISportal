...
OM_NAMESPACE = '{http://www.opengis.net/om/1.0}'
...
def findCoverageNode( coverageRoot, name ):
   possibleNodes = coverageRoot.findall('./%sObservationOfferingList'  % (SOS_NAMESPACE))
   for node in possibleNodes:
      coverageName = node.find('./%sname' % (SOS_NAMESPACE)).text
...
def createCache(server, capabilitiesXML, coverageXML):
   ...
   utils.saveFile(SERVERCACHEPATH + server['name'] + '-DescribeSensor' + FILEEXTENSIONXML, coverageXML)
   ...
   for service in root.findall('./%sServiceIdentification' % (OWS_NAMESPACE)):
      serverTitle = service.find('./%sTitle' % (OWS_NAMESPACE)).text
      serverAbstract = service.find('./%sAbstract' % (OWS_NAMESPACE)).text if service.find('./%sAbstract' % (OWS_NAMESPACE)) is not None else None
      
   # get all the offerings
   for product in root.findall('./%sContents/%sObservationOfferingList/%sObservationOffering' % (SOS_NAMESPACE,SOS_NAMESPACE,SOS_NAMESPACE)):
      sensorId = product.attrib['%sid' % (GML_NAMESPACE)]

      # we don't want the first item which all sensors grouped together
      if sensorId is 'network-all':
         continue   

      sensorName = product.find('./%sname' % (GML_NAMESPACE)).text
      ...
      if utils.blackfilter(sensorName, productBlackList):
         ...
         for prop in product.findall('./%sobservedProperty' % (SOS_NAMESPACE)):
            name = prop.attrib['%shref' % (XLINK_NAMESPACE)]
            # title = layer.find('./%sTitle' % (WMS_NAMESPACE)).text
            # abstract = layer.find('./%sAbstract' % (WMS_NAMESPACE)).text
            # temporal = False
            

            if name not in server['observedProperty']:
               print "NOTICE: Property '" + name + "' found on SOS server but not in local config file, ignoring."
               continue
...
def createStylesArray(layer):
   ...
   for style in layer.findall('./%sStyle' % (WMS_NAMESPACE)):      
      styles.append({"Name": style.find('./%sName' % (WMS_NAMESPACE)).text,
                         ...
                         "LegendURL": style.find('./%sLegendURL/%sOnlineResource' % (WMS_NAMESPACE,WMS_NAMESPACE)).get('%shref' % (XLINK_NAMESPACE)),
...
utils.updateCaches(createCache, dirtyCaches,  sosMarkers.sensors, SERVERCACHEPATH, MASTERCACHEPATH, CACHELIFE)
