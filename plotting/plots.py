...
def get_plot_data(json_request, plot=dict(), download_dir="/tmp/"):
   ...
   if plot_type in ("hovmollerLat", "hovmollerLon"):
      ...
      for s in series:
         ...
         try:
            if irregular:
               ...
               extractor = TransectExtractor(ds['threddsUrl'], time_bounds, "Time", extract_area=bbox, extract_variable=coverage, extract_depth=depth, outdir=download_dir, status_details=status_details)
      ...
      for s in series:
         ...
         extractor = TransectExtractor(wcs_url, [time], "time", extract_area=bbox,status_details=status_details, extract_variable=coverage, extra_slices=True)
      ...
      for s in series:
         ...
         data_request = "TransectExtractor('{}',{},extract_area={},extract_variable={})".format(wcs_url, time, bbox, coverage)
         debug(3, u"Requesting data: {}".format(data_request))
         extractor = TransectExtractor(wcs_url, [time], "time", extract_area=bbox, extract_variable=coverage, status_details=status_details, outdir=download_dir, extra_slices=True)
         files = extractor.getData()
         if files:
