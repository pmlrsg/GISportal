...
class TransectExtractor(Extractor):
	...
	def getMaxSlices(self, offset_vectors):
		...
		max_slices = int(self.thredds_max_request / slice_megabytes) - 2
	...
	def mergeFiles(self, files):
		...
		variable = new_file.createVariable(self.extract_variable,
			first_file.variables[self.extract_variable].dtype, ('time', 'lat', 'lon'), zlib=False)
