...
e[1]),at:c.join(" ")};b=a.extend({},a.ui.dialog.prototype.options.position,b)}else b=a.ui.dialog.prototype.options.position;(g=this.uiDialog.is(":visible"))||this.uiDialog.show();this.uiDialog.position(b);g||this.uiDialog.hide()}})})(jQuery);var opec=opec||(opec={}),map;opec.middlewarePath="/alphaservice";opec.wcsLocation=opec.middlewarePath+"/wcs2json/wcs?";opec.wfsLocation=opec.middlewarePath+"/wfs2json/wfs?";opec.cache={};opec.cache.wmsLayers=[];opec.cache.wfsLayers=[];
...
OpenLayers.ProxyHost=opec.middlewarePath+"/proxy?url=";
...
opec.createRefLayers=function(){function a(a,b){var c=new OpenLayers.Layer.Vector(a.name,{protocol:new OpenLayers.Protocol.HTTP({url:b,format:new OpenLayers.Format.GML}),strategies:[new OpenLayers.Strategy.Fixed],projection:lonlat,styleMap:a.style});c.controlID="refLayers";c.setVisibility(!1);c.displayTitle=a.displayTitle;c.title=a.displayTitle;map.addLayer(c);opec.leftPanel.addLayerToGroup(c,$("#refLayerGroup"))}opec.leftPanel.addGroupToPanel("refLayerGroup","Reference Layers",$("#opec-lPanel-reference"));
...
format:new OpenLayers.Format.KML({extractStyles:!0,extractAttributes:!0})})});b.controlID="refLayers";b.selected=!0;b.displayTitle="The Black Sea (KML)";map.addLayer(b);opec.leftPanel.addLayerToGroup(b,$("#refLayerGroup"));map.numRefLayers=map.getLayersBy("controlID","refLayers").length};
opec.createOpLayers=function(){$.each(opec.cache.wmsLayers,function(a,b){if("undefined"!==typeof b.server&&"undefined"!==typeof b.wmsURL&&"undefined"!==typeof b.wcsURL&&"undefined"!==typeof b.serverName){var c=b.wmsURL,d=b.wcsURL,f=b.serverName;$.each(b.server,function(a,b){b.length&&$.each(b,function(b,g){if(g.Name&&""!==g.Name){var k=new opec.MicroLayer(g.Name,g.Title,g.Abstract,g.FirstDate,g.LastDate,f,c,d,a,g.EX_GeographicBoundingBox);opec.checkNameUnique(k);$("#layers").multiselect("addItem",
...
a.type&&((new Date(a.output.global.time)).getTime(),m={id:"wcsgraph"+Date.now(),title:"WCS Hovm\u00f6ller Test Graph"},hovmoller(m,a.output))},error:function(a,b,c){gritterErrorHandler({type:"wcs data",request:a,errorType:b,exception:c,url:this.url})}})})},destroy:function(){}};opec.window.addWindow(graphCreator);opec.window.createMetadata=function(a){if(null!==a){$("#metadata-"+a.name).length&&$("#metadata-"+a.name).extendedDialog("close");var b={name:a.name,displayTitle:a.displayTitle,northBoundLat:a.exBoundingBox.NorthBoundLatitude,eastBoundLon:a.exBoundingBox.EastBoundLongitude,southBoundLat:a.exBoundingBox.SouthBoundLatitude,westBoundLon:a.exBoundingBox.WestBoundLongitude,addDateRange:a.temporal,productAbstract:a.productAbstract,firstDate:a.firstDate,lastDate:a.lastDate};$(document.body).append(opec.templates.metadataWindow(b));
...
break;case "box":k=a;$("#graphcreator-bbox").size()&&$("#graphcreator-bbox").val(k.toBBOX(5,!1));$("#dispROI").html("<h3>Rectangular ROI</h4>");$("#dispROI").append('<canvas id="ROIC" width="100" height="100"></canvas>');c=document.getElementById("ROIC");c=c.getContext("2d");c.lineWidth=4;c.fillStyle="#CCCCCC";j=e>f?90/e:90/f;c.fillRect(5,5,e*j,f*j);c.strokeRect(5,5,e*j,f*j);$("#dispROI").append("<p>Width: "+e.toPrecision(4)+"&deg; ("+i.toPrecision(4)+" km)</p>");$("#dispROI").append("<p>Height: "+
...
e.toPrecision(4)+"&deg; ("+i.toPrecision(4)+" km)</p>");$("#dispROI").append("<p>Height: "+f.toPrecision(4)+"&deg; ("+g.toPrecision(4)+" km)</p>");$("#dispROI").append("<p>Projected Area: "+d.toPrecision(4)+" km<sup>2</sup></p>");break;case "polygon":g=c.getVertices();$("#dispROI").html("<h3>Custom Polygon ROI</h4>");$("#dispROI").append('<canvas id="ROIC" width="100" height="100"></canvas>');c=document.getElementById("ROIC");c=c.getContext("2d");c.lineWidth=4;c.fillStyle="#CCCCCC";j=e>f?90/e:90/
...
opec.contextMenu.getCurrentStyles=function(a){var b=map.getLayersByName(a.attr("id"))[0];if("opLayers"!=b.controlID)return[];var c=[],a=b.styles.slice();a.push({Name:"Remove Style",Abstract:"",LegendURL:"",Width:0,Height:0});$.each(a,function(a,f){c["Layer Styles"+a]={name:f.Name,className:f.Name==b.params.STYLES?"styleSelected":"",callbackName:"Layer Styles "+f.Name,callback:function(){var a=map.getLayersByName($(".selectedLayer:visible").attr("id"))[0];a.mergeNewParams({styles:"Remove Style"==f.Name?
"":f.Name});console.log(f.Name);updateScalebar(a)}}});return c};opec.contextMenu.getCurrentElevation=function(a){var b=map.getLayersByName(a.attr("id"))[0],c=[];$.each(b.elevationCache,function(a,f){c["Layer Elevation "+a]={name:parseFloat(f).toFixed(3)+" "+b.elevationUnits,className:f==b.params.ELEVATION?"elevationSelected":"",callbackName:"Layer Elevation "+f,callback:function(){map.getLayersByName($(".selectedLayer:visible").attr("id"))[0].mergeNewParams({elevation:f})}}});return c};
opec.contextMenu.showGraphCreator=function(){return{name:"Show Graph Creator",callback:function(){opec.window.createGraphCreator()}}};OpenLayers.Map.prototype.cesiumLoaded=null;OpenLayers.Map.prototype.sampleLayers=["no3","chl","v_wind","CRW_SST"];OpenLayers.Map.prototype.enabledDays=[];OpenLayers.Map.prototype.getCapabilities=[];OpenLayers.Map.prototype.numBaseLayers=0;OpenLayers.Map.prototype.numRefLayers=0;OpenLayers.Map.prototype.numOpLayers=0;OpenLayers.Map.prototype.helperMessages=[];OpenLayers.Map.prototype.microLayers=[];OpenLayers.Map.prototype.layerStore=[];OpenLayers.Map.prototype.ROI_Type="";
...
OpenLayers.Map.prototype.getLayerData=function(a,b){$.ajax({type:"GET",url:"./cache/layers/"+a,dataType:"json",asyc:!0,cache:!1,success:function(a){opec.createOpLayer(a,b);opec.addOpLayer(b.name)},error:function(a,b,f){gritterErrorHandler({type:"layer cache",request:a,errorType:b,exception:f,url:this.url})}})};
